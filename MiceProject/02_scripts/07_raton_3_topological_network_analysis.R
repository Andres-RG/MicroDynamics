# Topological network analysis raton_3 === --- ===
##---------------------------
# Se define la ruta
setwd("MiceProject/")
##---------------------------
...
load(file = "03_out/data/mice_multilayer_network_properties_mouse_3.RData") # mice_ml_properties_s3
##---------------------------
ml_subject3 <- list(
  basal_subject3_aggregate,
...
Nbasal_subject3      <- net_inference(basal_subject3_aggregate, "aracne")
Nfatdiet_subject3    <- net_inference(fatdiet_subject3_aggregate, "aracne")
Nrecover1_subject3   <- net_inference(recover1_subject3_aggregate, "aracne")
Nvancomycin_subject3 <- net_inference(vancomycin_subject3_aggregate, "aracne")
Nrecover2_subject3   <- net_inference(recover2_subject3_aggregate, "aracne")
Ngentamicin_subject3 <- net_inference(gentamicin_subject3_aggregate, "aracne")
Nrecover3_subject3   <- net_inference(recover3_subject3_aggregate, "aracne")
##
fatdiet_period_s3    <- list(Nbasal_subject3,
                             Nfatdiet_subject3,
                             Nrecover1_subject3)
vancomyci_period_s3  <- list(Nrecover1_subject3,
                             Nvancomycin_subject3,
                             Nrecover2_subject3)
gentamicin_period_s3 <- list(Nrecover2_subject3,
                             Ngentamicin_subject3,
                             Nrecover3_subject3)
treatments <- c(1,2,3)
##
# Propiedades de las redes multicapa
mice_ml_properties_s3 <- rbind(ml_properties(fatdiet_period_s3, treatments),
                            ml_properties(vancomyci_period_s3, treatments),
                            ml_properties(gentamicin_period_s3, treatments))
##
Stage = c(rep("Fat diet", length(fatdiet_period_s3)),
          rep("Vancomycin", length(vancomyci_period_s3)),
          rep("Gentamicin", length(gentamicin_period_s3))
)
mice_ml_properties_s3 <- cbind(mice_ml_properties_s3, Stage)
## Se genera un objeto que tiene las propiedades de las redes multicapa por 
## cada tratamiento
## Se guarda objeto .RData
# save(mice_ml_properties_s3, file = "03_out/data/mice_multilayer_network_properties_mouse_3.RData")
##---------------------------
names(mice_ml_properties_s3)
# Graficas
## mean degree === === === ==
plot_mean_degree <- ggplot(
  mice_ml_properties_s3,
  aes(x = Treatments,
      y = Mean_degree,
      col = Stage) 
) +
...
##
# jpeg("03_out/plots/plot_mice_multilayer_network_analysis_mouse_3.jpeg",
...
grid.arrange(plot_mean_degree,
             ...
             plot_modularity,
             ncol = 3,
             top = "Multilayer network analysis mouse 3")
# dev.off()